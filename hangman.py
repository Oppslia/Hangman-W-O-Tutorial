...
while word != "completed" or strikes < (strikeAmount - 1):
    ...
    if errorHandling(guess) == True: #If the input is valid
        if guess not in word.lower(): # If the guess is not in the word
            if guess in guessedLetters:
                print(f'\nStrikes {f'{strikes}:{strikeAmount}'}\nAvailable letters:\n{" ".join(letterList)}\n')
                continue
            ...
            if len(dynamicString)>0: #If they have guessed something correctly, and they enter a wrong input print all letters in their places.
                print(dynamicString) 
            else: #If the string is blank(Only happens when guessing wrong before the first letter is guessed correctly) since dynamicString is only set after guessing the first letter.
            ...
            dynamicString = assembleString(word,guess) # resets the string to blank for each guess
